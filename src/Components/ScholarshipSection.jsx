...
function ScholarshipSection() {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                ...
                if (res.ok && data && data.data && data.data.data) {
                    SetScholarships(data.data.data); 
                } else {
                    SetErrorMessage("Something went wrong or data format is incorrect");
                }
            } catch (error) {
    ...
    return (
        <div className="grid gap-y-8 md:grid-cols-2 lg:grid-cols-3">
            ...
            {!isLoading && Array.isArray(scholarships) && scholarships.map((scholarship) => (
                <Card key={scholarship.id} scholarship={scholarship} />
            ))}

            {!isLoading && !errorMesage && scholarships.length === 0 && (
                <p className="col-span-full text-center text-slate-500">لا توجد منح متاحة حالياً.</p>
            )}
